...
exports.carUpdate = async (req, res) => {
  ...
  try {
    ...
    await conn.execute(
      `UPDATE CAR SET 
        CARTYPE=:type, CARBRAND=:brand, CARMODEL=:model, CARPLATENO=:plateno,
        CARCOLOUR=:colour, CARSEAT=:seat, CARFEE=:fee, CARSTATUS=:status
       WHERE CARID=:id`,
      {
        type: CARTYPE,
        brand: CARBRAND,
...
exports.deleteCar = async (req, res) => {
  ...
  try {
    conn = await getConnection();
    await conn.execute(`DELETE FROM CAR WHERE CARID=:id`, { id }, { autoCommit: true });
