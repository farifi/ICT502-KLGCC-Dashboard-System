...
const Car = () => {
  ...
  const carColumns = [
    ...
    { header: "Fee (RM)", key: "CARFEE", render: row => `RM ${Number(row.CARFEE).toFixed(2)}` },
    { header: "Status", key: "CARSTATUS" },
    {
      ...
      render: (row) => (
        <div className="table-actions">
          ...
          <button className="delete" onClick={() => handleDelete(row.CARID)}>🗑</button>
        </div>
      )
    }
  ];
  ...
  return (
    <div className="default-page">
      <div className="default">
        ...
        <div className="default-main">
          ...
          <div className="table-header">
            <button className="add-btn" onClick={() => setIsAddOpen(true)}>+ Add Car</button>
          </div>

          <div className="default-content">
            <h2 className="section-title">Available Cars</h2>
            <Table columns={carColumns} data={availableCars} />
            <Pagination currentPage={availablePage} totalPages={availableTotalPages} onPageChange={setAvailablePage} />

            <h2 className="section-title cancelled-title">Unavailable Cars</h2>
            <Table columns={carColumns} data={unavailableCars} />
            <Pagination currentPage={unavailablePage} totalPages={unavailableTotalPages} onPageChange={setUnavailablePage} />
          </div>
        </div>
      </div>

      <Modal isOpen={isAddOpen} title="Add Car" onClose={() => setIsAddOpen(false)}>
        <AddCarForm
      ...
      <Modal isOpen={isEditOpen} title="Edit Car" onClose={() => setIsEditOpen(false)}>
        <EditCarForm
          car={selectedCar}
